...
HISTORY_FILE = os.path.join(DIR, "chat_history.json")
...
def determine_model(prompt):
    p = prompt.lower()
    vision_keywords = ["scan screen", "look at my screen", "analyze screen", "screenshot", "what is on my screen"]
    if any(k in p for k in vision_keywords):
        return "llava", "VISION"
...
class AgentEngineWorker(QThread):
    ...
    def __init__(self, history_turns, prompt):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            if is_vision and not capture_screen():
            ...
            if is_vision and os.path.exists(SCREENSHOT_PATH):
                with open(SCREENSHOT_PATH, "rb") as f:
                    img_bytes = f.read()
                messages.append({
                    "role": "user",
                    "content": self.prompt + "\nAnalyze this screen frame precisely.",
                    "images": [img_bytes]
                })
            else:
            ...
            try:
                ...
                fallback = "llama3" if not is_vision else "llava"
...
class HoloHUD(QWidget):
    ...
    def process_command(self):
        ...
        self.worker = AgentEngineWorker(active_history, text)
